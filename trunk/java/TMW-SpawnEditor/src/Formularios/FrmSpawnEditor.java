...
import BancoDeDados.Banco_Itens;
import BancoDeDados.Banco_Monstros;
import BancoDeDados.Banco_Monstros.Dados_Monstro;
import BancoDeDados.Banco_Monstros.Dados_Monstro.Banco_Sprites;
...
public class FrmSpawnEditor extends javax.swing.JFrame {
    public FrmSpawnEditor(String PastaDoLocalhost) {
		try {
			...
			super.setIconImage((new ImageIcon(getClass().getResource("/Imagens/Botoes/sbl_halloween.png"))).getImage());
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 public static String pastaDoLocalhost="";
    public static Banco_Itens bdProds;
    public static Banco_Monstros bdMOBs;
   ...
   private void initComponents() {
      ...
      jPanel2Layout.setHorizontalGroup(
         ...
         .addGroup(jPanel2Layout.createSequentialGroup()
            ...
            .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
               .addComponent(txtNomeSumonico, javax.swing.GroupLayout.DEFAULT_SIZE, 415, Short.MAX_VALUE)
               .addComponent(txtNomeTitulo, javax.swing.GroupLayout.DEFAULT_SIZE, 415, Short.MAX_VALUE)))
      );
      ...
      jPanel14.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Descrição", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Dialog", 1, 12))); // NOI18N
      ...
      jPanel13Layout.setHorizontalGroup(
         ...
         .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel13Layout.createSequentialGroup()
            ...
            .addGroup(jPanel13Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
               ...
               .addComponent(jPanel12, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.DEFAULT_SIZE, 549, Short.MAX_VALUE))
            .addContainerGap())
      );
      ...
      tgbDirecaoEsquerda.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagens/Botoes/ceta-oeste.png"))); // NOI18N
      ...
      tgbDirecaoDireita.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagens/Botoes/ceta-leste.png"))); // NOI18N
      ...
      tgbDirecaoAbaixo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagens/Botoes/ceta-sul.png"))); // NOI18N
      ...
      tgbDirecaoCentral.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagens/Botoes/cetas.png"))); // NOI18N
    ...
    private Vector addItemVector(int IDTem, double PerctDeDrops){
        ...
        //Linha.addElement(new ImageIcon(bdProds.getItemPorID(IDTem).getIconeImagem()));
        //Linha.addElement(IDTem);
        //Linha.addElement(bdProds.getItemPorID(IDTem).getNomeTitulo());//Se � um
        Linha.addElement("<html>"+
            "<table><tr><td><img align=\"middle\" src=\"file://"+
                bdProds.getItemPorID(IDTem).getIconeEndereco()+
            "\"></td><td>"+
                "<b>"+IDTem+":</b> "+bdProds.getItemPorID(IDTem).getNomeTitulo()+"<br/>"+
                "<small>"+
                "V:"+bdProds.getItemPorID(IDTem).getPrecoDeVenda()+"GP "+
                "C:"+bdProds.getItemPorID(IDTem).getPrecoDeCompra()+"GP "+
                "("+bdProds.getItemPorID(IDTem).getPoderEfeito()+")"+
                "</small>"+
            "</td></tr></table>"
        );//Se � um
    ...
    //#########################################################################
        ...
        Dados_Monstro Monstro = bdMOBs.getMonstroPorOrdem(ordem);
        ...
        if(camBaseXML.getSpritePNGdados()!=null && FileClass.seExiste(camBaseXML.getSpritePNGdados().getEnderecoPNG())){
            int s1=0,x1=0,y1=0;
    ...
    private void exibirLotePorOrdemDeMonstro(int OrdemDeMonstro) {
        int Prods = bdMOBs.getMonstroPorOrdem(OrdemDeMonstro).getDropsCont();
        Vector Dados = new Vector();
        if(Prods>=1){
            for(int P=0;P<Prods;P++){
                Dados.add(
                    addItemVector(
                        bdMOBs.getMonstroPorOrdem(OrdemDeMonstro).getDropPorOrdem(P).getID(),
                        ((double)bdMOBs.getMonstroPorOrdem(OrdemDeMonstro).getDropPorOrdem(P).getPercentual())/100.0
                    )
                );
            }
        }
        ...
        tblItens.setModel(new DefaultTableModel(Dados,NomesDeColuna2) {
            ...
            public void setValueAt(Object aValue, int row, int column) {
                ...
                if(column==0){
                    ...
                    if(bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).getDropPorOrdem(row).getID()!=ID2){
                        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).getDropPorOrdem(row).setID(ID2);
                        darAuteracao(true);
                    }
                }else if(column==1){
                    Double PercDeDrops2= Double.parseDouble(aValue.toString().trim())*100;
                    if(bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).getDropPorOrdem(row).getPercentual()!=PercDeDrops2){
                        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).getDropPorOrdem(row).setPercentual(PercDeDrops2.intValue());
                        darAuteracao(true);
                    }
                }
                darAuteracao(true);
        ...
        tblItens.getTableHeader().getColumnModel().getColumn(0).setCellEditor(new MyComboBoxEditor(bdProds.getTitutos()));
    ...
    private void AbrirRegistro(int Ordem) {
        if(Ordem<0) return;
        txtNomeTitulo.setText(bdMOBs.getMonstroPorOrdem(Ordem).getNomeTitulo());
        txtNomeSumonico.setText(bdMOBs.getMonstroPorOrdem(Ordem).getNomeSumonico());
        txtDescricao.setText(bdMOBs.getMonstroPorOrdem(Ordem).getDescricao());
        
        sldNivel.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getNivel());
        sldRanger1.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getRange1());
        sldHP.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getHP());
        sldSP.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getSP());
        sldEXP.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getExp());
        sldJob.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getJob());
        sldAtaque1.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getAtaque1());
        sldAtaque2.setMinimum(sldAtaque1.getValue());
        sldAtaque2.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getAtaque2());
        sldDefFisico.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getDefesaFisica());
        sldDefMagico.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getDefesaMagica());

        final Vector xmlAnimacoes = new Vector();
        for(int s=0;s<bdMOBs.getMonstroPorOrdem(Ordem).getContSprites();s++){
            Banco_Sprites Sprites =bdMOBs.getMonstroPorOrdem(Ordem).getSpritePorOrdem(s);
        ...
        for(int s=0;s<bdMOBs.getMonstroPorOrdem(Ordem).getContSons();s++){
            Vector oggAudio = new Vector();
            oggAudio.addElement(bdMOBs.getMonstroPorOrdem(Ordem).getSomPorOrdem(s).getEvent());
            oggAudio.addElement(bdMOBs.getMonstroPorOrdem(Ordem).getSomPorOrdem(s).getEndereco());
        ...
        sldEstatusForca.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getEstatusForca());
        sldEstatusAgilidade.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getEstatusAgilidade());
        sldEstatusVitalidade.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getEstatusVitalidade());
        sldEstatusInteligencia.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getEstatusInteligencias());
        sldEstatusDestreza.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getEstatusDestresa());
        sldEstatusSorte.setValue(bdMOBs.getMonstroPorOrdem(Ordem).getEstatusSorte());

        int Modo=bdMOBs.getMonstroPorOrdem(Ordem).getComportamento();
    ...
    private void SalvarRegistro() {
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setNomeTitulo(txtNomeTitulo.getText().trim());
        lblVisualizacao.setText(txtNomeTitulo.getText().trim());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setNomeSumonico(txtNomeSumonico.getText().trim());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setDescricao(txtDescricao.getText().trim());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setNivel(sldNivel.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setRange1(sldRanger1.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setHP(sldHP.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setSP(sldSP.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setExp(sldEXP.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setJob(sldJob.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setAtaque1(sldAtaque1.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setAtaque2(sldAtaque2.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setDefesaFisica(sldDefFisico.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setDefesaMagica(sldDefMagico.getValue());

        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setEstatusForca(sldEstatusForca.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setEstatusAgilidade(sldEstatusAgilidade.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setEstatusVitalidade(sldEstatusVitalidade.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setEstatusInteligencia(sldEstatusInteligencia.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setEstatusDestresa(sldEstatusDestreza.getValue());
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setEstatusSorte(sldEstatusSorte.getValue());
        ...
        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setComportamento(Modo);

        bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).setDropsReset();
        if(tblItens.getRowCount()>=1){
            //int Prods = bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).getDropsCont();
            ...
            if(Prods>=1){
                for(int P=0;P<Prods;P++){
                    ...
                    bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).addDrop(ID, Percet);
    ...
    private void txtNomeSumonicoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtNomeSumonicoFocusLost
        ...
        if(!bdMOBs.getMonstroPorOrdem(cmbIDs.getSelectedIndex()).getNomeSumonico().equals(NomeSumonico)){
    ...
    private void btnItemAdicionarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnItemAdicionarActionPerformed
        if(tblItens.getRowCount()<8){
            ...
            //FrmPrincipal.setAvisoEmEstatus("<html><font color=\"#FF0000\">AVISO:</FONT> Voc� atengiu o limite m�ximo de itens por lote!");
				DialogClass.showErro("<html><font color=\"#FF0000\">AVISO:</FONT> Voc� atengiu o limite m�ximo de itens por lote!","AÇÃO NEGADA");
        }
    ...
    private void btnItemRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnItemRemoverActionPerformed
        if(tblItens.getRowCount()>=1){
            ...
            if(X>=0 && Y>=0){
                ...
                ImagemClass ItemImagem = new ImagemClass(bdProds.getItemPorID(IDicon).getIconeImagem());
                ...
                //FrmPrincipal.setAvisoEmEstatus("<html><font color=\"#FF0000\">AVISO:</FONT> Selecione o Item que deseja remover!");
					 DialogClass.showErro("<html><font color=\"#FF0000\">AVISO:</FONT> Selecione o Item que deseja remover!","AÇÃO NEGADA");
            }
        }else{
            //FrmPrincipal.setAvisoEmEstatus("<html><font color=\"#FF0000\">AVISO:</FONT> N�o h� itens para serem remividos deste lote!");
			  DialogClass.showErro("<html><font color=\"#FF0000\">AVISO:</FONT> Não há itens para serem remividos deste lote!","AÇÃO NEGADA");
        }
    ...
    */
    /*public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                FrmSpawnEditor dialog = new FrmSpawnEditor(new javax.swing.JFrame(), true);
